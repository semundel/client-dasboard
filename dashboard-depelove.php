	...
	<table>
		<thead>
			<tr>
				...
				<th data-sort="string">White Label</th>
				<th data-sort="int">Targeted Fans</th>
				<th data-sort="int">Current Fans</th>
				<th data-sort="int">Targeted </br>Sign-Ups</th>
				<th data-sort="int">Current </br>Sign-Ups</th>
			...
			<tr>
				...
				<?php
					...
					foreach ($data as $key => $value) { 
						...
						echo "
							<td>
								<form method='post' id='editsignform-".$key."' class='editype'>
									<input type='text' id='editsignup-".$key."' name='pageSignup' value='".$signup."' disabled='disabled' style='background:#fff; border: 1px solid #fff;width: 65px;padding: 3px 5px;height: auto;margin: 0px;float: left;'/>
						...
						if(empty($Redme)){ 
							echo "<td><span style='width:75px;float:left'>N.A</span> 
						...
						if($like2 >= $like1 && $totalsignups >= $signup){
							echo "<td><img src='img/green-alert.gif'/></td> ";
						}
						else{
							echo "<td><img src='img/red-alert.gif'/></td> ";							
						}
						
						echo "<td style='cursor:pointer;'><img class='add' data-parameter='".$key."'  src='img/add.png'/></td> ";
						echo "
							<td class='dispop' id='redme-".$key."' style='border:0px;float:left;padding:0px;margin:0px;'>
								<div class='redhek'>
				...
				<div class="dispop" id="tampil-<?php echo $key;?>">
					Guaranted Fans <span style="margin-right:10px;color:#2781CF;"><?php echo $garfans;?></span>
					...
					<div id="References-<?php echo $key;?>" class="refdes" data-parameter="<?php echo $key;?>">
						<div class="refdesdex">
							<form action="" method="post" id="updatechages-<?php echo $key;?>" class="updatechages" data-parameter="<?php echo $key;?>">	
								...
								$refren = unserialize($ref);
								for($i=0; $i<count($refren['Reference']); $i++){ ?>
										<input type="hidden" name="UrlLink[]" value="<?php echo $refren['url'][$i]; ?>"/></div>
										<input type="hidden" name="Referencename[]" value="<?php echo $refren['Reference'][$i]; ?>"/></div>
								<?php } ?>
